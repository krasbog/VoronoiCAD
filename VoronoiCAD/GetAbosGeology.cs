...
{
    public class AbosGeology
    {
        public static void getXlsGeoData(ref List<CIngGeolElem> lIGE, ref List<CGeolSkv> lSkvs,
            ref ABOSparams abosParams, ref FundamParams fundamParams, ref List<CRzFromXLS> lRz)
        {
         ...
         _AcAp.Application.ShowModalDialog(_AcAp.Application.MainWindow, dlg);
            ...
            //Имя файла .txt Лиры с генерируемыми сваями
            string fileNameNewLiraPileTxt = dlg.LiraNewPileFileName;

            //Файл .xls с реакциями Rz
            string xlsRzfileName = dlg.RzFileName;

            //Файл .txt исходный Лира
            string txtOrigFileName = dlg.OrigFileName;

            //Файл .txt результирующий Лира
            string txtResultFileName = dlg.ResultFileName;

            if (xlsRzfileName != "" && result == DialogResult.OK)
            {
                lRz = InputData.getRzFromXLS(xlsRzfileName);
            }
                ...
                if (XLSfileName != "" && result == DialogResult.OK)
            {
                ...
                lIGE = InputData.getIGEtable(XLSfileName);

                //Список скважин.
                //Отметки верха и низа слоев из таблицы исх. данных
                //Отметки уровня подземных вод
                //Обозначения скважин
                lSkvs = InputData.getSkvs(XLSfileName);

                //Параметры ABOS интерполяции
                abosParams = ABOSparams.getABOSparams(XLSfileName);

                //Параметры свайного фундамента
                fundamParams = FundamParams.getFundamParams(XLSfileName);
                fundamParams.fileNameNewLiraPileTxt = fileNameNewLiraPileTxt;
                fundamParams.fileNameOrigLiraTxt = txtOrigFileName;
                fundamParams.fileNameResultLiraTxt = txtResultFileName;

            }
        }
        public static void getDwgGeoData(SelectionSet acSSet, ref List<CGeolSkv> lSkvs,
        ...
        public static void getAbosGeology(ref List<CdMatrix> AbosZs, ref List<List<Point3d>> AbosPtss, ref List<string> layerNames,
            ref ABOSparams abosParams, ref List<CGeolSkv> lSkvs,
            ref double x1, ref double x2, ref double y1, ref double y2)
        {
            ...
            for (int k = 1; k < AbosZs.Count - 1; k++)
            {
                for (int i = 0; i < AbosZs[k].size1(); i++)
                {
                    for (int j = 0; j < AbosZs[k].size2(); j++)
                    {

                        if (AbosZs[k][i, j] < AbosZs[k + 1][i, j]) AbosZs[k + 1][i, j] = AbosZs[k][i, j];
                    }
                }
        ...
        public static void DrawBS(ref List<CPile> piles)
        {
            //Отрисовка поверхности границы глубины сжимаемой толщи
            DatabaseCAD.do_addLayer("BC", 1);
            var p1 = new Polygon();//точки полигона для триангуляции
            int marker1 = 0;
            List<Point3d> BCpts = new List<Point3d>();
            foreach (var pile in piles)
            {
                BCpts.Add(new Point3d(pile.pileCell.ptPile.X, pile.pileCell.ptPile.Y,
                    pile._layersUnderFL._layers[pile._layersUnderFL._layers.Count-1].Bottom));
                p1.Add(new TriangleNet.Geometry.Vertex(pile.pileCell.ptPile.X, pile.pileCell.ptPile.Y, marker1));
                marker1++;
            }
            ...
            foreach (var triang in mesh.Triangles)
            {
                ...
                DatabaseCAD.do_addFace(pt0, pt1, pt2, "BC");


            }
        }
        public static void DrawAbosGeology(ref List<List<Point3d>> AbosPtss, ref List<string> layerNames)
        {
